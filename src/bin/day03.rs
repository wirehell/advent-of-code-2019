...
fn extend_wire(wire: &mut Wire, extension: Ext) {
    ...
    let dir = match extension {
        Ext::U(_)|Ext::D(_) => Vertical,
        Ext::R(_) | Ext::L(_) => Horizontal,
    };

//    println!("NP: {} {}", new_point.x, new_point.y);
    wire.push(Line {from: last_point, to: new_point, dir });
}
...
fn get_intersection(line1 :&Line, line2:&Line) -> Option<Point> {
    return match (&line1.dir, &line2.dir) {
        (Horizontal, Vertical) => get_intersection_2(line1, line2),
        (Vertical, Horizontal) =>  get_intersection_2(line2, line1),
        _ => None
    };
}
